...
> **Go module 结构。** open-stream-saver 以仓库根 module `github.com/w1977-0/open-stream-saver` 发布，CLI 源码有意保留在 `cli/` 目录。安装时应使用 CLI 的 package path，并固定到根版本 tag，例如 `@v0.3.1`；不要在 `go install` 的版本位置使用 `cli/vX.Y.Z` 形式。
