...
> **Go module layout.** open-stream-saver is released as the root module `github.com/w1977-0/open-stream-saver`; the CLI source intentionally remains under `cli/`. Install the CLI by its package path and pin a root release tag, for example `@v0.3.2`. Do not use a `cli/vX.Y.Z` suffix in the `go install` version.
